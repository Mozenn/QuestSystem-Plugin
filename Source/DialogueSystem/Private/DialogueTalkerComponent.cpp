...
void UDialogueTalkerComponent::StartDialogue(AActor* ListenerActor)
{
	if (ensure(DialogueGraph))
	{
		...
		case EDialogueSceneType::SCENE:
		{
			...
			if (SceneNode)
			{
				CurrentListenerInterface->OnNewScene_Implementation(SceneNode->SceneTexture);
			}
		...
		case EDialogueSceneType::INTERACTIVE:
		{
			...
			if (InteractiveNode)
			{
				...
				CurrentListenerInterface->OnNewInteractiveScene_Implementation(InteractiveSceneWidget);
			}
